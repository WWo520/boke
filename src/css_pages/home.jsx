...
export default function Home() {
  ...
  }, [pageNum, currentPage, totalPages, navigate]);
        ...
        <div className={styles.heroContainer}>
          ...
          <div className={styles.heroCategories}>
            {categories.slice(0, 4).map((cat) => (
              <Link key={cat.slug} to={`/category/${cat.slug}`} className={styles.heroCatCard}>
          ...
          <div className={styles.sidebarSection}>
            ...
            <div className={styles.tagCloud}>
              {popularTags.map((tag) => (
                <Link
                  key={tag.tag}
                  to={`/search?q=${tag.tag}&type=posts`}
        ...
        <main className={styles.main}>
          <div className={styles.contentHeader}>
            ...
            <div className={styles.contentTabs}>
              <Link to="/" className={`${styles.tab} ${!selectedCat ? styles.tabActive : ''}`}>最新</Link>
              <Link to="/rankings" className={styles.tab}>热榜</Link>
              <Link to="/questions" className={styles.tab}>问答</Link>
            </div>
          </div>
                  ...
                  <div className={styles.postLink}>
                    ...
                    <div className={styles.postContent}>
                      ...
                      <div className={styles.postMeta}>
                        <Link to={`/u/${post.author?.name}`} className={styles.authorLink}>
                          {post.author?.name}
                        </Link>
        ...
        <aside className={styles.rightSidebar}>
            ...
            <ul className={styles.hotList}>
              {hotPosts.map((post, index) => (
                <li key={post.id} className={styles.hotItem}>
                  ...
                  <Link to={`/post/${post.slug}`} className={styles.hotLink}>
            ...
            <ul className={styles.tagRankList}>
              {popularTags.slice(0, 10).map((tag, index) => (
                <li key={tag.tag} className={styles.tagRankItem}>
                  ...
                  <Link to={`/search?q=${tag.tag}&type=posts`} className={styles.tagRankLink}>
                    {tag.tag}
                  </Link>
          ...
          {featuredPosts.length > 0 && (
            <div className={styles.sidebarSection}>
              ...
              <ul className={styles.featuredList}>
                {featuredPosts.map((post) => (
                  <li key={post.id} className={styles.featuredItem}>
                    <Link to={`/post/${post.slug}`} className={styles.featuredLink}>
              ...
              <Link to="/rankings" className={styles.viewMore}>
